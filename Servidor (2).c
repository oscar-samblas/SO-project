...
int CrearPartida(TablaPartidas tabla, char organizador[20], int socket, int id){
	tabla[numFilas].id = id;	
...
int DameJugadoresPartida(TablaPartidas tabla, int id, char j1[20], char j2[20], char j3[20], char j4[20], char j5[20], char j6[20], char j7[20], char j8[20], char j9[20], char j10[20]){
	int i = 0;
	int encontrado = 0;
	...
	if (encontrado == 0){
		...
		strcpy(j1, tabla[i].JugadoresPartida[0].Usuario);
		strcpy(j2, tabla[i].JugadoresPartida[1].Usuario);
		strcpy(j3, tabla[i].JugadoresPartida[2].Usuario);
		strcpy(j4, tabla[i].JugadoresPartida[3].Usuario);
		strcpy(j5, tabla[i].JugadoresPartida[4].Usuario);
		strcpy(j6, tabla[i].JugadoresPartida[5].Usuario);
		strcpy(j7, tabla[i].JugadoresPartida[6].Usuario);
		strcpy(j8, tabla[i].JugadoresPartida[7].Usuario);
		strcpy(j9, tabla[i].JugadoresPartida[8].Usuario);
		strcpy(j10, tabla[i].JugadoresPartida[9].Usuario);
		return 0;
	}
...
int AddJugadorPartida(TablaPartidas tabla, char j[20], int socket, int id){
	int encontrado = 0;
	int found=0;
	int i = 0;
	int m=0;
	while ((i<numFilas) && (encontrado==0)){
		if (tabla[i].id == id){
			encontrado = 1;
			while((m<tabla[i].num_Jugadores) &&(found ==0)){
				if (strcmp(tabla[i].JugadoresPartida[m].Usuario,j) == 0)
					found =1;
				else
					j++;
			}
	...
	if ((encontrado == 1) && (found == 1)){
		tabla[i].JugadoresPartida[tabla[i].num_Jugadores].Socket = socket;
		strcpy(tabla[i].JugadoresPartida[tabla[i].num_Jugadores].Usuario, j);
		tabla[i].num_Jugadores++;
		return i;
	}
	else 
		return -1;
...
int DameOrganizadorPartida(TablaPartidas tabla, int id, char organizador[20]){
	int encontrado = 0;
	int i=0;
	while ((i<numFilas) && (encontrado == 0)){
		if (tabla[i].id == id)
	...
	if (encontrado == 0){
		...
		strcpy(organizador, tabla[i].JugadoresPartida[0].Usuario);
		return i;
	}
}
	...
	while (end == 0){
		...
		if (codigo == 0){	//Peticion de desconexion
			...
			if (correcto ==0){
				...
				p = strtok(NULL,"/");
				idPartida = atoi(p);
			//	int creado = CrearPartida(Tabla_P,organizador,sock_conn,idPartida);
				//if (creado == -1){
			...
			//	else{
				...
				//			printf("Socket: %d \n", SocketDeLaPersonaInvitada);
						//	added = AddJugadorPartida(Tabla_P, nombre_Usuario, SocketDeLaPersonaInvitada, idPartida);
							//if (added == -1){
							//	printf("Error al añadir a la partida");
						//	}
						//	else{
						//		printf("Jugador añadido a partida. Nombre: %s\n", nombre_Usuario);
					//		}
					//	}
					//	p = strtok(NULL,"/");
				//	}
			...
			//int creador = DameOrganizadorPartida(Tabla_P,idPartida, organizador);
			...
			//	printf("Organizador: %s \n", organizador);
				...
				//		strcpy(respuesta_SI_NO,p);
					...
					//	sprintf(respuesta, "8/%d/%s/%s/",idPartida, invitada, respuesta_SI_NO);
				//	}
			//	}
		//	}
		}
